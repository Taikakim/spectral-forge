> **Status (2026-04-27): IMPLEMENTED.** All 10 tasks merged on `feature/next-gen-modules-plans`. `PunchModule` with two modes (`Direct`, `Inverse`) sharing one sidechain-peak-driven carve kernel; Direct detects peaks in the SC, Inverse inverts the SC into a dedicated `peak_scratch` buffer first (does NOT clobber the smoothing follower state in `current_carve_depth`). Greedy local-max peak detector with `min_dist` cluster suppression and fixed `[_; 256]` on-stack candidate scratch (`MAX_PEAKS = 32`, `MAX_DRIFT_SITES = 64`). Per-bin carve+follower kernel with 5 ms attack and HEAL-controlled release (τ=150 ms nominal); neighbour amp-fill via triangle weight in `[0, half_w]` bins around each peak. Sub-bin pitch-fill via `drift_accum` (slew-rate limited at 0.005 bin/hop, hard-clamped to ±0.5 bins) applies per-hop phase rotation `Δφ = (π/2)·d` (assumes `OVERLAP = 4`). Per-slot mode persistence via `Arc<Mutex<[PunchMode; 9]>>` (mirrors `slot_future_mode` shape) with serde round-trip and `try_lock` audio-thread dispatch; persist key count bumped 25 → 26. UI extends the existing `Mode` row block to handle both Future and Punch in a single visible-when-applicable row, mirroring the established button-pattern. Calibration probes (test/feature-gated) cover Direct AMOUNT default/max, MIX max, and Inverse AMOUNT max — verifying probe is mode-agnostic at the per-bin level. Punch is the first module to default `wants_sidechain: true` in its `ModuleSpec`, signalling the UX hint added in Phase 1; SC is already routed by default at the slot level (`ScChannel::Follow`, `slot_sc_gain_db = 0.0`) so no extra hookup was needed. Self-Punch deferred per audit. The code is the source of truth; this plan is kept for history. See [../STATUS.md](../STATUS.md).
