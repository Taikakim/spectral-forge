...
fn soft_clip_clamps_high_magnitude_when_on() {
    ...
    // fft = 32 → t_lin at 0 dB = 8, ceiling = 32. Hot bin (mag = 64) clamps under ceiling.
    let mut bins = [Complex::new(64.0_f32, 0.0); 32];
    apply_soft_clip(&mut bins, 32, 0.0, 32);
    for k in 0..32 {
        assert!(bins[k].norm() < 32.0,
...
fn soft_clip_passes_low_magnitude_unchanged() {
    ...
    // through bit-exact. fft = 16, threshold_db = 0 → t_lin = 4; mag = 0.1 < 4.
    let mut bins = [Complex::new(0.1_f32, 0.0); 16];
    let original = bins[0];
    apply_soft_clip(&mut bins, 16, 0.0, 16);
    for c in &bins {
